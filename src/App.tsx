...
export default function App() {
    ...
    const handleImportData = (newSectors: any[], newScoresByDate: any) => {
    ...
    return (
        <div className={`min-h-screen ${theme.bg} ${theme.text} transition-colors duration-200`}>
            {/* Main content */}
            <main className="mx-auto max-w-7xl px-4 pt-3 sm:px-6">
                <button
                    onClick={() => setDrawerOpen(true)}
                    className={`fixed top-4 right-4 rounded-lg border ${theme.border} ${theme.button} px-4 py-2 text-sm transition-colors`}
                >
                    ⚙️ Configuración
                </button>
                {/* Selector de fecha */}
            ...
            <div className={`rounded-2xl p-4 sm:p-8`}>
                <div className="flex flex-col items-center gap-4">
                    ...
                    {/* SVG de la rueda */}
                    <div className="relative overflow-hidden rounded-xl" style={{ touchAction: 'none' }}>
                        <WheelChart
            ...
            {/* Instrucciones */}
            <p className={`fixed bottom-4 left -4 text-center text-xs ${theme.textMuted} max-w-md me-10`}>
            ...
            {(zoomPan.scale !== 1 || zoomPan.translateX !== 0 || zoomPan.translateY !== 0) && (
                <button
                    ...
                    className={`fixed bottom-4 right-4 rounded-lg border ${theme.border} ${theme.button} px-4 py-2 text-sm transition-colors`}
                >
                    🔄 Resetear zoom
                </button>
            )}
